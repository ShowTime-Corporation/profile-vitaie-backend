...
public class UserRequestDTO {
    ...
    @NotBlank(message = "Name can not be empity")
    @Size(max = 200, message = "Name can not be more than 200 characters")
    private String first_name;

    @NotBlank(message = "Lastname can not be empity")
    @Size(max = 200, message = "Lastname can not be more than 200 characters")
    private String last_name;

    @NotBlank(message = "Email can not be empity")
    @Email(message = "Invalid email format")
    @Size(max = 300)
    private String user_email;

    @NotBlank(message = "Password can not be empity")
